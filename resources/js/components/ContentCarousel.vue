...
<template>
    <Carousel class="relative w-full" :opts="{
            ...
            <CarouselContent class="-ml-2">
                <CarouselItem v-for="(content, index) in contents" :key="content.id"
                    ...
                    <Card class="relative rounded-lg overflow-hidden aspect-[5/8]">
                        <CardContent>
                            ...
                            <div class="absolute z-10 mb-4 bottom-0 left-0 w-full text-center">
                                <slot name="item-extra" :item="content" :index="index" />
                                <p class="text-[10px] font-medium">{{ content.content.title }}</p>
                            </div>
                        </CardContent>
                    </Card>
                </CarouselItem>
