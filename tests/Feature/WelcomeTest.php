...
test('guest sees login button in header', function () {
    $response = $this->get(route('home'));
    $response->assertStatus(200)->assertInertia(fn (Assert $page) =>
        $page->component('Welcome')->has('auth')->where('auth.user', null)
    );
});
...
test('authenticated user sees dashboard button in header', function () {
    ...
    $response->assertInertia(fn (Assert $page) =>
        $page->component('Welcome')->has('auth.user', fn (Assert $userProp) =>
            $userProp->where('name', $user->name)->where('email', $user->email)->etc()
        )
    );
});
